...
export default function MovieCard(props) {
  ...
  const toggleWatched = (isWatched) => {
    API.toggleWatched(user.email, props.movie.title, isWatched).then(() =>
      props.updateUsers()
    );
  };

  const updateRating = (event) => {
    API.setRating(user.email, props.movie.title, event.target.value).then(() =>
      props.updateUsers()
    );
  };
    ...
    <Grid item xs={location.pathname === "/collections" ? 12 : 6}>
      ...
      >
        <CardHeader
          ...
          subheader={"Release Date: " + props.movie.release_date}
        />
        <CardMedia
          ...
          {location.pathname === "/collections" ? (
            <>
              <IconButton
                ...
                flex="none"
                onClick={() => toggleWatched(!props.movie.watched)}
              >
